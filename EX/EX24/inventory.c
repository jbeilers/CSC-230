...
int itemComp( void const *pA, void const *pB )
{
  ...
  // Compare the length of the items pointed to by itemA and itemB.)
    ...
    if (itemA->mtype == imperial) {
        ImperialLength a = itemA->length.ival;
        A = INCHES_TO_METERS * (a.inches + (double)a.numerator/(double)a.denominator);
    } else {
        A = itemA->length.mval;
    }
    if (itemB->mtype == imperial) {
        ImperialLength b = itemB->length.ival;
        B = INCHES_TO_METERS * (b.inches + (double)b.numerator/(double)b.denominator);
    } else {
        B = itemB->length.mval;
    }
...
int main( int argc, char *argv[] )
{
  ...
  for ( int i = 0; i < icount; i++ ) {
    // Read an item and store it in element i of ilist.
      ...
      char mtype;
      if (fscanf(fp, "%d %c", &item->sku, &mtype) != 2)
        invalid();

      if (mtype == 'm') {
          item->mtype = metric;
          if (fscanf(fp, "%lf", &item->length.mval) != 1)
              invalid();
      }
      else if (mtype == 'i') {
          item->mtype = imperial;
          if (fscanf(fp, "%d %d / %d\n",
              &item->length.ival.inches, &item->length.ival.numerator, &item->length.ival.denominator) != 3)
              invalid();
      }
      ilist[i] = *item;
  }
  ...
  for ( int i = 0; i < icount; i++ ) {
    ...
    // as a mixed numeral if it's metric.
      Item item = ilist[i];
    printf("%d ", item.sku, item.mtype);
      if (item.mtype == metric)
          printf("%c %f\n", 'm', item.length.mval);
      else
          printf("%c %d %d / %d\n",
              'i', item.length.ival.inches, item.length.ival.numerator, item.length.ival.denominator);
  }
