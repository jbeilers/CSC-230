...
void readCity( FILE *fp, struct City *cty ) {
    ...
    char name[21];
    fscanf(fp, "%s %lf %lf", name, &(cty->lat), &(cty->lon));
    strcpy( cty->name, name );
...
int main( int argc, char *argv[] )
{
  ...
  struct City (*cities)[n] = malloc( n * sizeof(struct City) );
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    // Find and report the nearest city.
      ...
      double closest = EARTH_RADIUS;
      struct City closestCity;
    for (int i = 0; i < n; i++) {
        struct City c = *(*cities + i);
        double dist = globalDistance(lat, lon, (c.lat), (c.lon));
        if (dist < closest) {
            closest = dist;
            closestCity = c;
        }
    }
      printf("%s %.2lf\n", closestCity.name, closest);
